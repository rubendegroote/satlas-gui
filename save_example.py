...
for i in range(3):
    ...
    x1 = np.linspace(-4000,4000,1200)
    y1 = spectra[0](x1) 
    y1 += np.abs(np.random.randn(x1.shape[0]) * np.sqrt(y1))

    x2 = np.linspace(-4000,4000,1200)
    y2 = spectra[1](x2) 
    ...
    np.savetxt('data1_'+str(i)+'.csv', np.column_stack((x1,y1)))
    np.savetxt('data2_'+str(i)+'.csv', np.column_stack((x2,y2)))


    a = Analysis(['data1_'+str(i)+'.csv', 'data2_'+str(i)+'.csv'])
    ...
    from satlas.analysisIO import save
    save(a,'test'+str(i))
